...
const wrapperClass = computed(() =>
    props.compact
        ? 'content-card-shell content-card-shell--compact flex'
        : [
  ...
  >
    ...
    <div class="flex min-w-0 flex-1 flex-col gap-3">
      <div class="flex items-start justify-between gap-3">
        <h3 class="line-clamp-2 text-base font-semibold tracking-[-0.02em] text-[var(--color-text)] md:text-[1.05rem]">
          {{ article.titleText }}
        </h3>
        <ArticleStatusBadge v-if="shouldShowStatusBadge && article.status" :status="article.status" />
      </div>

      <ArticleSummary v-if="article.summary.text" :summary="article.summary" />
